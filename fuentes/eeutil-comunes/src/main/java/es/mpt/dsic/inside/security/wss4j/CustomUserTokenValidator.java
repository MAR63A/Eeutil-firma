...
public class CustomUserTokenValidator implements Validator {
  ...
  /**
   * Validate the credential argument. It must contain a non-null UsernameToken. A CallbackHandler
   * implementation is also required to be set.
   * 
   ...
   * 
   * If the password is null it queries a hook to allow the user to validate UsernameTokens of this
   * type.
   * 
  ...
  /**
   * Verify a UsernameToken containing a password of some unknown (but specified) password type. It
   * does this by querying a CallbackHandler instance to obtain a password for the given username,
   * and then comparing it against the received password. This method currently uses the same logic
   * as the verifyPlaintextPassword case, but it in a separate protected method to allow users to
   * override the validation of the custom password type specific case.
   * 
   ...
   * CallbackHandler instance to obtain a password for the given username, and then comparing it
   * against the received password. This method currently uses the same logic as the
   * verifyDigestPassword case, but it in a separate protected method to allow users to override the
   * validation of the plaintext password specific case.
   * 
   * @param usernameToken The UsernameToken instance to verify
   * @throws WSSecurityException on a failed authentication.
   */
  ...
  /**
   * Verify a UsernameToken containing a password digest. It does this by querying a CallbackHandler
   * instance to obtain a password for the given username, and then comparing it against the
   * received password.
   * 
   * @param usernameToken The UsernameToken instance to verify
   * @throws WSSecurityException on a failed authentication.
  ...
  protected void verifyDigestPassword(UsernameToken usernameToken, RequestData data)
    ...
    if (usernameToken.isHashed()) {
      ...
      if (passwordsAreEncoded) {
        passDigest =
            UsernameToken.doPasswordDigest(nonce, createdTime, Base64.decode(origPassword));
      } else {
  ...
  /**
   * Verify a UsernameToken containing no password. This does nothing - but is in a separate method
   * to allow the end-user to override validation easily.
   * 
   * @param usernameToken The UsernameToken instance to verify
   * @throws WSSecurityException on a failed authentication.
   */
