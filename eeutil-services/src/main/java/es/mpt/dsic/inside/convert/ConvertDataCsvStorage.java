...
import es.mpt.dsic.csvstorage.admin.model.AltaAplicacionResponse;
import es.mpt.dsic.csvstorage.admin.model.Aplicacion;
import es.mpt.dsic.csvstorage.admin.model.BajaAplicacionResponse;
import es.mpt.dsic.csvstorage.admin.model.InfAdicional;
import es.mpt.dsic.csvstorage.admin.model.InfAdicionalAltaAppResponse;
import es.mpt.dsic.csvstorage.admin.model.ListaAplicacionesResponse;
import es.mpt.dsic.inside.model.aplicacion.AplicacionObject;


public class ConvertDataCsvStorage {
  ...
  public static Map<String, String> convertInfAdicional(Aplicacion data) {
    Map<String, String> retorno = new HashMap<String, String>();
    retorno.put("validarDocumentoENI", data.isValidarDocumentoENI().toString());
    return retorno;
  }
  ...
  public static Aplicacion convertData(AplicacionObject data) {
    ...
    retorno.setIdAplicacionPublico("aap_" + data.getIdentificador());
    retorno.setResponsable(data.getResponsable());
    retorno.setTelefono(data.getTelefono());
    if (data.getAditionalData() != null
        && data.getAditionalData().containsKey("validarDocumentoENI")) {
      retorno.setValidarDocumentoENI(
          Boolean.valueOf(data.getAditionalData().get("validarDocumentoENI")));
    }
    retorno.setSerialNumberCertificado(data.getSerialNumberCertificado());
