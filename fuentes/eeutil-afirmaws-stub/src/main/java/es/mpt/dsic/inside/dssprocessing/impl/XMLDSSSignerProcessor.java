 ...
 *
 */
public abstract class XMLDSSSignerProcessor extends DSSSignerProcessor {
  ...
  public byte[] getUpgradedSignature(ResponseBaseType verifyResponse) {
    ...
    // En las firmas XADES DETACHED y ENVELOPED la firma actualizada se almacena en
  ...
  /*
   * @Override public ContenidoFirmado getSignedData(ResponseBaseType verifyResponse, final byte[]
   * sign) throws ContentNotExtractedException {
   * 
   ...
   * 
   * 
   * int i = 0; SignedDataRefType signedDataRefConEncoding = null;
   * 
   * // Nos quedamos con el que tenga un elemento "Encoding" dentro while (i <
   * signedDataRefList.size() && signedDataRefConEncoding == null) { if
   * (signedDataRefList.get(i).getEncoding() != null) { signedDataRefConEncoding =
   * signedDataRefList.get(i); } i++; }
   * 
   ...
   * signedDataRefConEncoding.getEncoding()); }
   * 
   * } catch (Exception e) { throw new ContentNotExtractedException
   * ("No se puede obtener el nodo que contiene el contenido firmado" + e.getMessage(), e); } }
   * 
   * if (contenidoFirmado == null) { throw new ContentNotExtractedException
   * ("No existe el nodo SignedDataInfo"); }
   * 
   ...
   */
  /*
   * protected ContenidoFirmado getContenidoFirmadoEnveloped (Document doc) throws
   * XPathFactoryConfigurationException, XPathExpressionException, UnsupportedEncodingException,
   * TransformerException { // Buscamos los nodos SignatureValue String xpathExpression = "//
   */
  /*
   * Signature";
   * 
   * 
   * // Los eliminamos XMLUtil.removeNodesFromDocument(xpathExpression, doc);
   * 
   * // Convertimos el documento a array de bytes. byte[] bytesDocumento =
   * XMLUtil.getBytesFromNode(doc, doc.getInputEncoding()).toByteArray(); ContenidoFirmado
   * contenidoFirmado = new ContenidoFirmado (); contenidoFirmado.setBytesDocumento(bytesDocumento);
   ...
   */
  /*
   * protected ContenidoFirmado getContenidoFirmadoDetached (Document doc, String xpathExpression,
   * String encodingSignedData) throws XPathFactoryConfigurationException, XPathExpressionException,
   * CharacterCodingException, TransformerException, UnsupportedEncodingException { // En este caso
   * obtenemos el nodoContenido para obtener el contenido del interior. Node nodoContenido =
   * XMLUtil.getNodeByXpathExpression(doc, xpathExpression);
   * 
   * ContenidoFirmado contenidoFirmado = new ContenidoFirmado();
   * 
   * // Obtenemos el contenido de ese nodo. // Si es hash if (XMLUtil.isHashMimeType(nodoContenido))
   * { contenidoFirmado.setHash(MiscUtil.decodeByEncoding(nodoContenido.getTextContent(),
   * encodingSignedData));
   * contenidoFirmado.setAlgoritmoHash(XMLUtil.getHashAlgorithm(nodoContenido)); // Si no es hash }
   * else { // Si lo que se ha firmado es un XML if (XMLUtil.isXMLMimeType(nodoContenido)) {
   ...
   * encodingSignedData).toByteArray(); contenidoFirmado.setBytesDocumento(bytesDocumento);
   * contenidoFirmado.setMimeDocumento("application/xml"); // Si no lo es } else { byte[]
   * bytesDocumento = MiscUtil.decodeByEncoding(nodoContenido.getTextContent(), encodingSignedData);
